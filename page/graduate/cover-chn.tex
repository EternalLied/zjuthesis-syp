% \thispagestyle{cover}

% \begin{center}
%     \zihao{-4} \songti
%     \begin{tabularx}{\textwidth}{l l >{\raggedleft}X l}
%         分类号：           & \underline{按中国图书分类法，学位办网上可查}  &
%         单位代码：         & \uline{\hfill 10335 \hfill} \\
%         密{\quad}级：      & \underline{注明密级与保密期限} &
%         学{\quad\quad}号： & \underline{\multido{}{4}{\quad}}
%     \end{tabularx}
% \end{center}


% \begin{center}
%     \includegraphics[width=0.5\paperwidth]{logo/zjuchar.pdf}
% \end{center}

% \vspace{-40pt}

% \begin{center}
%     \zihao{-1} \songti%
%     \TitleTypeNameCover
% \end{center}

% \ifthenelse{\equal{\DepartmentLines}{1}}
% {
%     %   DepartmentLines == 1
%     \vskip 20pt
% }
% {
%     %   DepartmentLines == 2
%     \vskip 10pt
% }

% \begin{center}
%     \includegraphics[width=0.15\paperwidth]{logo/zju.pdf}
% \end{center}

% \ifthenelse{\equal{\DepartmentLines}{1}}
% {
%     %   DepartmentLines == 1
%     \vskip 20pt
% }
% {
%     %   DepartmentLines == 2
%     \vskip 10pt
% }

% % Define multi line title



% \begin{center}
%     \bfseries \zihao{-2}
%     \begin{tabularx}{.8\textwidth}{>{\fangsong}l X<{\centering}}
%         % If titles are too long for one row, please manually hardcode them in
%         % multiple lines, as the english title example here.
%         \ifthenelse{\equal{\TitleLines}{1}}
%         {
%             % TitleLines == 1
%             中文论文题目：&  \uline{\hfill \fangsong \Title{} \hfill} \\
%             ~ & \uline{\hfill} \\
%         }
%         {
%             % TitleLines == 2
%             中文论文题目：&  \uline{\hfill \fangsong \TitleLineOne{} \hfill} \\
%             ~            & \uline{\hfill \fangsong \TitleLineTwo{} \hfill} \\
%         }

%         \ifthenelse{\equal{\TitleEngLines}{1}}
%         {
%             % TitleEngLines == 1
%             英文论文题目：&  \uline{\hfill \TitleEng{} \hfill} \\
%             ~ & \uline{\hfill} \\
%         }
%         {
%             % TitleEngLines == 2
%             英文论文题目：&  \uline{\hfill \TitleEngLineOne{} \hfill} \\
%             ~            & \uline{\hfill \TitleEngLineTwo{} \hfill} \\
%         }
%     \end{tabularx}
% \end{center}

% \ifthenelse{\equal{\DepartmentLines}{1}}
% {
%     %   DepartmentLines == 1
%     \vskip 20pt
% }
% {
%     %   DepartmentLines == 2
%     \vskip 10pt
% }

% \begin{center}
%     \zihao{4}
%     \begin{tabularx}{.6\textwidth}{>{\fangsong}l >{\fangsong}X<{\centering}}
%         \ifthenelse{\equal{\BlindReview}{true}}%
%         {%
%             申请人姓名： & \uline{\hfill} \\
%             指导教师：   & \uline{\hfill} \\
%             合作导师：   &  \uline{\hfill} \\
%         }
%         {%
%             申请人姓名： & \uline{\hfill \StudentName \hfill} \\
%             指导教师：   & \uline{\hfill \AdvisorName \hfill} \\
%             合作导师：   &  \uline{\hfill \ColaboratorName \hfill} \\
%         }
%         \ifthenelse{\equal{\Type}{design}}
%         {%
%             专业学位类别：  &  \uline{\hfill \Major \hfill} \\
%             专业学位领域：  &  \uline{\hfill \Topic \hfill} \\
%         }
%         {%
%             学科(专业)：  &  \uline{\hfill \Major \hfill} \\
%             研究方向：  &  \uline{\hfill \Topic \hfill} \\
%         }
%         \ifthenelse{\equal{\DepartmentLines}{1}}
%         {%   DepartmentLines == 1
%         所在学院：   &  \uline{\hfill \Department \hfill} \\
%         }
%         {%   DepartmentLines == 2
%         所在学院：   &  \uline{\hfill \DepartmentLineOne \hfill} \\
%                     &  \uline{\hfill \DepartmentLineTwo \hfill} \\
%         }
%     \end{tabularx}
% \end{center}

% \ifthenelse{\equal{\DepartmentLines}{1}}
% {
%     %   DepartmentLines == 1
%     \vskip 15pt
% }
% {
%     %   DepartmentLines == 2
%     \vskip 10pt
% }

% \begin{center}
%     \zihao{-3} \bfseries
%     \begin{tabularx}{.5\textwidth}{>{\fangsong}l >{\fangsong}X<{\centering}}
%         论文递交日期 & \uline{\SubmitDate}
%     \end{tabularx}
% \end{center}

% 公爵师弟的模板---------------------------
\thispagestyle{cover}

\begin{center}
    \zihao{-4} \songti
    \begin{tabularx}{\textwidth}{l l >{\raggedleft}X l}
        分类号：           & \underline{TP393}  &
        单位代码：         & \uline{\hfill10335    \hfill} \\   %10335
        密{\quad}级：      & \underline{无 \hfill} &
        学{\quad\quad}号： & \uline{\hfill 22332074 \hfill} 
    \end{tabularx}
\end{center}


\begin{center}
    \includegraphics[width=0.5\paperwidth]{logo/zjuchar.pdf}
\end{center}

\vspace{-40pt}

\begin{center}
    \zihao{-1} \songti%
    \TitleTypeNameCover
\end{center}

\vskip 20pt

\begin{center}
    \includegraphics[width=0.15\paperwidth]{logo/zju.pdf}
\end{center}

\vskip 20pt

\begin{center}
    \bfseries \zihao{3}
    \begin{tabularx}{0.95\textwidth}{>{\fangsong}l X<{\centering}}
        % If titles are too long for one row, please manually hardcode them in
        % multiple lines, as the english title example here.
        中文论文题目：       &  \uline{\hfill\fangsong 面向分布式训练的集合通信 \hfill} \\
        				    &  \uline{\hfill\fangsong 与通算并行优化研究      \hfill} \\
        英文论文题目：       &  \uline{\hfill \fontsize{11pt}{14pt}\selectfont
                            {On Optimization of Collective Communication and }
                            \hfill} \\
        				    &  \uline{\hfill \fontsize{11pt}{14pt}\selectfont
                            {Computation–Communication Overlap for Distributed Training}
                            \hfill} \\
    \end{tabularx}
\end{center}

\vskip 20pt

\begin{center}
    \zihao{4}
    \begin{tabularx}{.65\textwidth}{>{\fangsong}l >{\fangsong}X<{\centering}}
        \ifthenelse{\equal{\BlindReview}{false}}%
        {%
            申请人姓名： & \uline{\hfill 沈奕澎 \hfill} \\
            指导教师：   & \uline{\hfill 徐金明 \hfill} \\
            合作导师：   &  \uline{\hfill  \hfill} \\
            \ifthenelse{\equal{\Type}{design}}
            {%
                专业学位类别：  &  \uline{\hfill 电子信息 \hfill} \\
                专业学位领域：  &  \uline{\hfill 控制工程 \hfill} \\
            }
            {%
                专业名称：  &  \uline{\hfill 网络空间安全 \hfill} \\
                研究方向：  &  \uline{\hfill 分布式训练加速 \hfill} \\
            }
            所在学院：   &  \uline{\hfill 控制科学与工程学院 \hfill} \\
        }
        {%
            申请人姓名： & \uline{\hfill \StudentName \hfill} \\
            指导教师：   & \uline{\hfill \AdvisorName \hfill} \\
            合作导师：   &  \uline{\hfill \ColaboratorName \hfill} \\
            \ifthenelse{\equal{\Type}{design}}
            {%
                专业学位类别：  &  \uline{\hfill \Major \hfill} \\
                专业学位领域：  &  \uline{\hfill \Topic \hfill} \\
            }
            {%
                专业名称：  &  \uline{\hfill \Major \hfill} \\
                研究方向：  &  \uline{\hfill \Topic \hfill} \\
            }
            所在学院：   &  \uline{\hfill \Department \hfill} \\
        }
    \end{tabularx}
\end{center}

\vskip 15pt

\begin{center}
    \zihao{-3} \bfseries
    \begin{tabularx}{.5\textwidth}{>{\fangsong}l >{\fangsong}X<{\centering}}
        论文递交日期 & \uline{\SubmitDate}
    \end{tabularx}
\end{center}
